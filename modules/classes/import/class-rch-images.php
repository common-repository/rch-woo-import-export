...
class RCH_Images extends \rch\import\base\RCH_Import_Base {
        ...
        public function prepare_images() {
                ...
                if ( $this->item_id ) {
                        ...
                        if ( ! $this->is_new_item && trim( strtolower( rch_sanitize_field( $this->get_field_value( 'rch_item_update_images' ) ) ) ) === "all" ) {
                                $this->prepare_old_attch();
                        }

                        $image_option = trim( strtolower( rch_sanitize_field( $this->get_field_value( 'rch_item_image_option', true ) ) ) );

                        $this->process_images( $image_option );

                        if ( ! $this->is_new_item && rch_sanitize_field( $this->get_field_value( 'rch_item_update_images' ) ) == "all" ) {
        ...
        private function is_search_existing() {
                ...
                return absint( rch_sanitize_field( $this->get_field_value( 'rch_item_search_existing_images', true ) ) ) === 1;
        }
        ...
        private function process_images( $method = "media_library" ) {
                ...
                $image_data = rch_sanitize_textarea( $this->get_field_value( 'rch_item_image_' . $method ) );
                ...
                if ( ( ! isset( $data[ 1 ] )) || ( isset( $data[ 1 ] ) && empty( $data[ 1 ] )) ) {
                        ...
                        $delim = rch_sanitize_field( $this->get_field_value( 'rch_item_image_' . $method . '_delim' ) );
                ...
                foreach ( $data as $index => $image ) {
                        ...
                        if ( empty( $attch_id ) && (($method === "media_library") || ($method !== "media_library" && $this->is_search_existing() ) ) ) {
                                ...
                                $media_id = $this->rch_get_image_from_gallery( $image );
                        ...
                        if ( empty( $attch_id ) ) {
                                $temp_id = false;
                                if ( $method === "local" ) {
                                        $temp_id = $this->rch_get_image_from_local( $image );
                                } elseif ( $method === "url" ) {
                                        $temp_id = $this->rch_get_image_from_url( $image );
                                }
                                if ( $temp_id !== false ) {
                        ...
                        if ( ! empty( $attch_id ) ) {
                                ...
                                $this->rch_set_image_meta( absint( $attch_id ), $index );
                        }
                }
        }
        ...
        private function prepare_image_option() {
                ...
                if ( absint( rch_sanitize_field( $this->get_field_value( 'rch_item_image_rename', true ) ) ) === 1 ) {

                        $new_names = rch_sanitize_field( $this->get_field_value( 'rch_item_image_new_name' ) );
                ...
                if ( absint( rch_sanitize_field( $this->get_field_value( 'rch_item_change_ext', true ) ) ) === 1 ) {

                        $new_ext = rch_sanitize_field( $this->get_field_value( 'rch_item_new_ext' ) );
        ...
        private function get_image_meta_values( $field = "" ) {
                ...
                if ( absint( rch_sanitize_field( $this->get_field_value( 'rch_item_set_image_' . $field, true ) ) ) !== 1 ) {
                        return $meta;
                }

                $value = rch_sanitize_textarea( $this->get_field_value( 'rch_item_image_' . $field ) );
                ...
                if ( ( ! isset( $data[ 1 ] )) || ( isset( $data[ 1 ] ) && empty( $data[ 1 ] )) ) {
                        $delim = rch_sanitize_field( $this->get_field_value( 'rch_item_set_image_' . $field . '_delim' ) );
        ...
        private function rch_set_image_meta( $attch_id = 0, $index = 0 ) {
        ...
        private function rch_get_image_from_gallery( $image_name = "" ) {
        ...
        private function rch_get_image_from_local( $filename = "" ) {

                if ( ( ! wp_is_writable( $this->target_dir )) || empty( $filename ) ) {
                        return;
                }

                $file = RCH_UPLOAD_TEMP_DIR . "/" . $filename;

                $file_info = pathinfo( $filename );

                $rename = rch_sanitize_field( $this->get_field_value( 'rch_item_image_rename', true ) );

                if ( $rename == 1 ) {

                        $new_filename = rch_sanitize_field( $this->get_field_value( 'rch_item_image_new_name' ) );
                ...
                $change_ext = absint( rch_sanitize_field( $this->get_field_value( 'rch_item_change_ext' ) ) );

                if ( $change_ext === 1 ) {

                        $new_file_ext = rch_sanitize_field( $this->get_field_value( 'rch_item_new_ext' ) );
                ...
                $this->import_log[] = '<strong>' . __( 'Warning', 'rch-woo-import-export' ) . '</strong> : ' . __( 'Image Upload failed', 'rch-woo-import-export' );
        ...
        private function rch_get_image_from_url( $image_url = "" ) {
                ...
                if ( is_wp_error( $attch ) ) {
                        $this->import_log[] = '<strong>' . __( 'Warning', 'rch-woo-import-export' ) . '</strong> : ' . $attch->get_error_message();
                        return false;
                }
        ...
        private function download_image( $file_url = "" ) {
                ...
                if ( ! $matches ) {
                        ...
                        return new WP_Error( 'invalid_image', __( 'Invalid image Extension', 'rch-woo-import-export' ) );
                }

                if ( $this->is_search_existing() ) {

                        $media_id = $this->rch_get_image_from_gallery( $original_name );
        ...
        private function remove_old_attch( $type = 'images' ) {
                ...
                if ( ! empty( $this->attach ) ) {
                        ...
                        $keep_images = absint( (rch_sanitize_field( $this->get_field_value( 'rch_item_keep_images', true ) ) ) === 1 );
        ...
        private function set_as_draft() {
                if ( ! $this->as_draft && absint( rch_sanitize_field( $this->get_field_value( 'rch_item_unsuccess_set_draft' ) ) ) === 1 ) {
                        $this->as_draft = true;
                }
        }
        ...
        private function set_gallary_images() {
                ...
                if ( ! empty( $this->images ) && absint( rch_sanitize_field( $this->get_field_value( 'rch_item_first_imaege_is_featured' ) ) ) === 1 ) {
                        ...
                        if ( $thumbnail_id === false ) {
                                ...
                                $this->import_log[] = '<strong>' . __( 'Warning', 'rch-woo-import-export' ) . '</strong> : ' . __( 'Error on try to set Featured Image', 'rch-woo-import-export' );
                        }
