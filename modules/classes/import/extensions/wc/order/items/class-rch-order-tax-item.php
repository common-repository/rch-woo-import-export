...
class RCH_Order_Tax_Item extends \rch\import\base\RCH_Import_Base {
    ...
    private function prepare_order_tax() {
        ...
        $delimiter = rch_sanitize_field($this->get_field_value('rch_item_order_item_taxes_delim'));

        $tax_rate = rch_sanitize_field($this->get_field_value('rch_item_order_item_tax_rate'));

        $current_tax_items = array();

        if (!empty($tax_rate)) {

            $tax_rate_amount = rch_sanitize_field($this->get_field_value('rch_item_order_item_tax_rate_amount'));
