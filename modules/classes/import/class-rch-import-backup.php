...
namespace rch\import\backup;

use WP_Error;
use rch\export\RCH_Export;

if (!defined('ABSPATH')) {
    die(__("Can't load this file directly", 'rch-woo-import-export'));
}

class RCH_Import_Backup {
    ...
    public function init_backup_services($import_type = "", $rch_taxonomy_type = "", $backup_dir = "") {
        ...
        $this->import_taxonomy_type = $rch_taxonomy_type;
    ...
    private function init_export() {
        ...
        $this->export = new \rch\export\RCH_Export();
        ...
        $this->item_template["rch_export_type"] = $this->import_type;

        $this->item_template["rch_taxonomy_type"] = $this->import_taxonomy_type;
    ...
    private function set_dir($backup_dir = "") {
        ...
        if ($backup_dir && wp_is_writable($backup_dir)) {
            ...
            return new \WP_Error('rch_import_error', __('Backup Directory is not writable', 'rch-woo-import-export'));
        }
        return true;
    }
    ...
    public function create_backup($item_id = 0, $is_new_item = false) {
        ...
        if (!isset($this->backup[$item_id])) {
            ...
            if ($is_success === false) {
                return new \WP_Error('rch_import_error', __('Fail To Generate Log', 'rch-woo-import-export'));
            }
            unset($is_success);
