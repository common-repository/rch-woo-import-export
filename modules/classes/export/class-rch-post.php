...
class RCH_Post extends \rch\export\engine\RCH_Export_Engine {
        ...
        protected function get_fields( $exclude_meta = array() ) {
                ...
                $image_fields = array(
                        "title"      => __( "Images", 'rch-woo-import-export' ),
                        "isFiltered" => false,
                        "data"       => array(
                                array(
                                        'name' => 'Image URL',
                ...
                $attachment_fields = array(
                        "title"      => __( "Attachments", 'rch-woo-import-export' ),
                        "isFiltered" => false,
                        "data"       => array(
                                array(
                                        'name' => 'Attachment URL',
                ...
                $author_fields = array(
                        "title"      => __( "Author", 'rch-woo-import-export' ),
                        "isExported" => false,
                        "data"       => array(
                                array(
                                        'name' => 'User ID',
                                ...
                                array(
                                        'name' => 'First Name',
                                        'type' => 'rch_cf_first_name',
                                ),
                                array(
                                        'name' => 'Last Name',
                                        'type' => 'rch_cf_last_name',
                                ),
                                array(
                                        'name' => 'Nickname',
                                        'type' => 'nickname',
                ...
                $other_fields = array(
                        "title" => __( "Other", 'rch-woo-import-export' ),
                        "data"  => array(
                                array(
                                        'name' => 'Status',
                                        'type' => 'status',
                ...
                $taxonomy_data = $this->get_taxonomies_by_post_type( $this->export_type, "rch_tax", false );

                $taxonomy = array(
                        "title" => __( "Taxonomy", 'rch-woo-import-export' ),
                        "data"  => $taxonomy_data
                );
                ...
                $addon_class = apply_filters( 'rch_prepare_post_fields', array(), $this->export_type );
        ...
        protected function get_meta_keys( $export_type = array() ) {
			...
			LIMIT %d";
                $limit = apply_filters( 'rch_postmeta_form_limit', 500 );
        ...
        protected function parse_rule( $filter = array() ) {
                ...
                if ( isset( $filter[ 'element' ] ) ) {
                        ...
                        $filter_data = apply_filters( 'rch_apply_post_filter', [], $this->export_type, $filter );
        ...
        protected function process_export() {
                ...
                $query = array(
                        ...
                        'posts_per_page' => isset( $this->template_options[ 'rch_records_per_iteration' ] ) ? absint( $this->template_options[ 'rch_records_per_iteration' ] ) : 50
                );
                ...
                $query = apply_filters( 'rch_pre_execute_post_query', $query );
        ...
        protected function process_items( $post_data = array() ) {
                ...
                $fields_data = (isset( $this->template_options[ 'fields_data' ] ) && trim( $this->template_options[ 'fields_data' ] ) != "") ? explode( "~||~", rch_sanitize_field( wp_unslash( $this->template_options[ 'fields_data' ] ) ) ) : array();
                                ...
                                if ( ! empty( $fields_data ) ) {
                                        ...
                                        foreach ( $fields_data as $field ) {
                                                ...
                                                switch ( $field_type ) {
                                                        ...
                                                        case 'id':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_id', $this->apply_user_function( $item->ID, $is_php, $php_func ), $item );
                                                                break;
                                                        case 'permalink':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_permalink', $this->apply_user_function( get_permalink( $item ), $is_php, $php_func ), $item );
                                                                break;

                                                        case 'post_type':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_type', $this->apply_user_function( $item->post_type, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'title':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_title', $this->apply_user_function( $item->post_title, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'content':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_content', $this->apply_user_function( $item->post_content, $is_php, $php_func ), $item );
                                                                break;
                                                        ...
                                                        case 'date':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_date', $this->apply_user_function( $post_date, $is_php, $php_func ), $item );
                                                        ...
                                                        case 'post_modified':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_modified_time', $this->apply_user_function( $post_modified_date, $is_php, $php_func ), $item );
                                                        ...
                                                        case 'parent':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_parent', $this->apply_user_function( $item->post_parent, $is_php, $php_func ), $item );
                                                                break;
                                                        ...
                                                        case 'parent_slug':
                                                                ...
                                                                if ( $item->post_parent != 0 ) {
                                                                        ...
                                                                        $rch_parent_posts = get_post_ancestors( $item->ID );

                                                                        if ( ! empty( $rch_parent_posts ) ) {

                                                                                $rch_slugs = array();

                                                                                foreach ( $rch_parent_posts as $rch_parent_post ) {

                                                                                        $the_post = get_post( $rch_parent_post );

                                                                                        if ( $the_post ) {
                                                                                                $rch_slugs[] = $the_post->post_name;
                                                                                        }

                                                                                        unset( $the_post );
                                                                                }

                                                                                $parent_slug = implode( "/", array_reverse( $rch_slugs ) );
                                                                        ...
                                                                        unset( $rch_parent_posts );
                                                                } else {
                                                                        $parent_slug = $item->post_parent;
                                                                }

                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_parent_slug', $this->apply_user_function( $parent_slug, $is_php, $php_func ), $item );

                                                                unset( $parent_slug );

                                                                break;
                                                        case 'comment_status':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_comment_status', $this->apply_user_function( $item->comment_status, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'ping_status':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_ping_status', $this->apply_user_function( $item->ping_status, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'template':

                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_template', $this->apply_user_function( get_post_meta( $item->ID, '_wp_page_template', true ), $is_php, $php_func ), $item );
                                                                break;

                                                        case 'order':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_menu_order', $this->apply_user_function( $item->menu_order, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'status':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_status', $this->apply_user_function( $item->post_status, $is_php, $php_func ), $item );
                                                                break;

                                                        case 'format':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_format', $this->apply_user_function( get_post_format( $item->ID ), $is_php, $php_func ), $item );
                                                                break;

                                                        case 'author_id':
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_author_id', $this->apply_user_function( $item->post_author, $is_php, $php_func ), $item );
                                                                break;
                                                        ...
                                                        case 'author_username':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_author', $this->apply_user_function( $username, $is_php, $php_func ), $item );
                                                        ...
                                                        case 'author_email':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_author_email', $this->apply_user_function( $user_email, $is_php, $php_func ), $item );

                                                                unset( $user_data, $user_email );

                                                                break;
                                                        case 'rch_cf_first_name':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_author_first_name', $this->apply_user_function( $user_fname, $is_php, $php_func ), $item );

                                                                unset( $user_data, $user_fname );

                                                                break;
                                                        case 'rch_cf_last_name':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_author_last_name', $this->apply_user_function( $user_lname, $is_php, $php_func ), $item );
                                                        ...
                                                        case 'slug':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_slug', $this->apply_user_function( $item->post_name, $is_php, $php_func ), $item );

                                                                break;

                                                        case 'excerpt':

                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_excerpt', $this->apply_user_function( $item->post_excerpt, $is_php, $php_func ), $item );
                                                        ...
                                                        case 'rch_cf':
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_meta', $this->apply_user_function( $meta_value, $is_php, $php_func ), $meta_key, $item );
                                                        ...
                                                        case 'rch_tax':
                                                                ...
                                                                $tax_name = isset( $field_option[ 'taxName' ] ) ? rch_sanitize_field( $field_option[ 'taxName' ] ) : "";
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_term', $this->apply_user_function( $this->export_data[ $field_name ], $is_php, $php_func ), $tax_name, $item );
                                                        ...
                                                        case 'attachment_alt':
                                                                ...
                                                                if ( ! (isset( $media[ $item->ID ] )) ) {
                                                                        ...
                                                                        $media[ $item->ID ] = $rch_media->get_media( $item->ID );
                                                                }
                                                                ...
                                                                if ( ! empty( $media[ $item->ID ] ) ) {
                                                                        ...
                                                                        $attch = $rch_media->get_attch( $media[ $item->ID ], $field_type, 'post' );
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_attach_' . $field_type, $this->apply_user_function( $attach_value, $is_php, $php_func ), $item );

                                                                unset( $rch_media, $attach_value );
                                                        ...
                                                        case 'image_alt':
                                                                ...
                                                                if ( isset( $media[ $item->ID ] ) ) {
                                                                        ...
                                                                        $image_data = $rch_media->get_images( $item->ID, $media[ $item->ID ], $field_type, 'post' );
                                                                ...
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_image_' . $field_type, $this->apply_user_function( $image_media, $is_php, $php_func ), $item );
                                                        ...
                                                        default:
                                                                $this->export_data[ $field_name ] = apply_filters( 'rch_export_post_field', $this->apply_user_function( "", $is_php, $php_func ), $item );
                                                                break;
                                                }
        ...
        public function posts_where( $data = "" ) {
                ...
                $this->item_where = apply_filters( 'rch_export_posts_where', $this->item_where, $this->item_join );
        ...
        public function posts_join( $data = "" ) {
                ...
                $this->item_join = apply_filters( 'rch_export_posts_join', $this->item_join );
        ...
        private function get_valid_element( $element = "" ) {
                ...
                switch ( $element ) {
                        ...
                        case 'order':
                                ...
                                break;
                        case 'template':
                                $return_data = 'rch_cf__wp_page_template';
                                break;
                        case 'format':
                                $return_data = 'rch_tax_post_format';
                                break;
                        default:
                                $return_data = $element;
                                break;
