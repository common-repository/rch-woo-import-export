...
class RCH_Media {
    ...
    public function get_media($media_id = 0) {
        ...
        $rch_attachments = get_posts(array(
            'post_type' => 'attachment',
            'posts_per_page' => -1,
            'post_parent' => $media_id,
        ));

        return $rch_attachments;
    }
    ...
    public function get_images($parent = 0, $attch = array(), $field_type = "", $type = "post") {
            ...
            if (!empty($image_gallery_data) && is_array($image_gallery_data)) {
                ...
                foreach ($image_gallery_data as $gallary_data) {
                    ...
                    if (!empty($gallary_data) && !in_array($gallary_data, $image_data)) {
                        ...
                        $rch_image = get_post($gallary_data);

                        if ($rch_image) {
                            $image_data[$gallary_data] = $rch_image;
                        }
                        unset($rch_image);
                    }
                }
        ...
        if (!empty($attch)) {
            ...
            foreach ($attch as $rch_image) {

                if (!in_array($rch_image, $image_data) && wp_attachment_is_image($rch_image->ID)) {

                    $image_data[$rch_image->ID] = $rch_image;
                }
            }
        }
        unset($attch);
        ...
        if (!empty($image_data)) {
            ...
            foreach ($image_data as $rch_attach) {

                $_value = $this->get_field_value(str_replace("image_", "", $field_type), $rch_attach, $type);
    ...
    public function get_attch($attachments = null, $field_type = "", $type = "post") {
        ...
        if ($attachments && is_array($attachments)) {
            foreach ($attachments as $rch_attch) {
                if (!wp_attachment_is_image($rch_attch->ID)) {
                    $media_attch[] = $rch_attch;
                }
            }
        }
