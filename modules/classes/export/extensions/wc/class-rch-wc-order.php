...
namespace rch\export\wc\order;

use rch\export\wc\product;
...
class RCH_WC_Order extends \rch\export\post\RCH_Post {
        ...
        private function get_order_item_standard_fields() {
                return array(
                        'title' => __("Item", 'rch-woo-import-export'),
                        'data' => array(
                                array(
                                        'name' => 'Product ID',
                                        'type' => 'wc-order',
        ...
        public function pre_process_fields(&$export_fields = array(), $export_type = array()) {
                ...
                $export_fields['customer'] = array(
                        'title' => __("Customer", 'rch-woo-import-export'),
                        'data' => array(
                                array(
                ...
                if (file_exists(RCH_EXPORT_CLASSES_DIR . '/extensions/wc/class-rch-wc-product.php')) {

                        require_once(RCH_EXPORT_CLASSES_DIR . '/extensions/wc/class-rch-wc-product.php');
                }

                $product = new \rch\export\wc\product\RCH_WC_Product(false);

                $product_fields = $product->product_metas();

                $export_fields['order_item_product'] = array(
                        'title' => __("Item => Product Data", 'rch-woo-import-export'),
                        'isExported' => false,
                        'data'       => $product->format_fields( $product_fields, 'wc-order-item', "wc-order-item-rch_cf" )
                );

                $export_fields['order_item_attribute'] = array(
                        'title' => __("Item => Attributes", 'rch-woo-import-export'),
                        'isExported' => false,
                ...
                $export_fields['orer_item_meta'] = array(
                        'title' => __("Item => Custom Fields", 'rch-woo-import-export'),
                        'isExported' => false,
                        'data' => $post_meta
                );
                ...
                $taxonomy = apply_filters('rch_pre_post_taxonomy_fields', $this->get_taxonomies_by_post_type(array('product', 'product_variation'), "wc-order-item-rch_tax", false));

                $export_fields['order_item_taxonomy'] = array(
                        'title' => __("Item => Taxonomy", 'rch-woo-import-export'),
                        'isExported' => false,
                        'data' => $taxonomy
                );
                ...
                $export_fields['taxes'] = array(
                        'title' => __("Taxes & Shipping", 'rch-woo-import-export'),
                        'isFiltered' => false,
                        'data' => array(
                                array(
                ...
                $export_fields['fees'] = array(
                        'title' => __("Fees & Discounts", 'rch-woo-import-export'),
                        'isFiltered' => false,
                        'data' => array(
                                array(
                ...
                $export_fields['notes'] = array(
                        'title' => __("Notes", 'rch-woo-import-export'),
                        'isFiltered' => false,
                        'data' => array(
                                array(
                                        'name' => 'Note Content',
                ...
                $export_fields['refund'] = array(
                        'title' => __("Refunds", 'rch-woo-import-export'),
                        'isFiltered' => false,
                        'data' => array(
                                array(
                                        'name' => 'Refund Name',
        ...
        private function available_billing_field_data() {
                ...
                return apply_filters('rch_export_order_billing_fields', $keys);
        }
        ...
        private function available_shipping_field_data() {
                ...
                return apply_filters('rch_export_order_shipping_fields', $keys);
        }

        public function init_export_process($post_data = array(), $template_options = array(), $export_id = 0) {

                $max_items = isset($template_options['rch_order_item_count']) ? $template_options['rch_order_item_count'] : 0;
                ...
                if (intval($max_items) > 0) {
                        ...
                        $template_options['rch_order_item_count'] = $this->max_items["line_item"];

                        if ($export_id > 0) {

                                global $wpdb;

                                $wpdb->update($wpdb->prefix . "rch_template", array('options' => maybe_serialize($template_options)), array('id' => absint($export_id)));
                        }
                }

                $this->order_item_data = array();

                $this->is_single_row = isset($template_options['rch_order_item_sigle_row']) && absint($template_options['rch_order_item_sigle_row']) == 1 ? false : true;

                $this->fill_empty_colunm = isset($template_options['rch_order_item_fill_empty']) && absint($template_options['rch_order_item_fill_empty']) == 1 ? true : false;
        }
        ...
        public function process_addon_data(&$export_data = array(), $field_type = "", $field_name = "", $field_option = array(), $item = null, $site_date_format = "") {
                ...
                if ($field_type) {
                        ...
                        $new_field_type = isset($field_option['field_type']) ? rch_sanitize_field($field_option['field_type']) : "";

                        $field_key = isset($field_option['field_key']) ? rch_sanitize_field($field_option['field_key']) : "";
