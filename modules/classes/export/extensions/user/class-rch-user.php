...
class RCH_User_Export extends \rch\export\engine\RCH_Export_Engine {
        ...
        protected function get_fields() {
                ...
                $metas = apply_filters('rch_pre_user_meta_fields', $this->get_meta_keys());


                $export_fields = array(
                        "standard" => apply_filters('rch_post_standard_fields', $standard_fields),
                        "meta"     => apply_filters('rch_post_meta_fields', $metas),
                        "other"    => apply_filters('rch_post_other_fields', $other_fields),
                );

                $addon_class = apply_filters('rch_prepare_user_fields', array(), $this->export_type);
                ...
                $meta_fields = array(
                        "title" => __("User Meta", 'rch-woo-import-export'),
                        "data"  => $meta_data
                );
        ...
        private function get_meta_keys() {
                ...
                $limit = apply_filters('rch_usermeta_form_limit', 500);
                        ...
                        switch ($filter['element']) {
                                ...
                                default:
                                        if (strpos($filter['element'], "rch_cf_") === 0) {
                                                ...
                                                $meta_key = $this->remove_prefix($filter['element'], "rch_cf_");
        ...
        protected function process_export() {
                ...
                $query = apply_filters('rch_pre_execute_user_query', $query);
                        ...
                        foreach ($user_data as $item_id) {
                                ...
                                foreach ($fields_data as $field) {
                                        ...
                                        switch ($field_type) {
                                                ...
                                                case 'id':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_id', $this->apply_user_function($item->ID, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_login':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_login', $this->apply_user_function($item->user_login, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_pass':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_pass', $this->apply_user_function($item->user_pass, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_email':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_email', $this->apply_user_function($item->user_email, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_nicename':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_nicename', $this->apply_user_function($item->user_nicename, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_url':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_url', $this->apply_user_function($item->user_url, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_activation_key':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_activation_key', $this->apply_user_function($item->user_activation_key, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_status':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_status', $this->apply_user_function($item->user_status, $is_php, $php_func), $item);
                                                        break;
                                                case 'display_name':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_display_name', $this->apply_user_function($item->display_name, $is_php, $php_func), $item);
                                                        break;
                                                case 'nickname':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_nickname', $this->apply_user_function($item->nickname, $is_php, $php_func), $item);
                                                        break;
                                                case 'first_name':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_first_name', $this->apply_user_function($item->first_name, $is_php, $php_func), $item);
                                                        break;
                                                case 'last_name':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_last_name', $this->apply_user_function($item->last_name, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_role':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_role', $this->apply_user_function(implode(",", $item->roles), $is_php, $php_func), $item);
                                                        break;
                                                case 'description':
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_description', $this->apply_user_function($item->description, $is_php, $php_func), $item);
                                                        break;
                                                case 'user_registered':
                                                        $user_registered = $this->get_date_field($date_type, strtotime($item->user_registered), $date_format, $site_date_format);
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_registered', $this->apply_user_function($user_registered, $is_php, $php_func), $item);
                                                        break;
                                                case 'rch_cf':
                                                        ...
                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_meta', $this->apply_user_function($meta_value, $is_php, $php_func), $meta_key, $item);
                                                ...
                                                default:
                                                        $defaults = apply_filters('rch_export_pre_user_default_field', "", $field_name, $field_option, $item);

                                                        $this->export_data[$field_name] = apply_filters('rch_export_user_default_field', $this->apply_user_function($defaults, $is_php, $php_func), $item);
